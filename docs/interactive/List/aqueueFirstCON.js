...
$(document).ready(function() {
  ...
  var interpret = {
    ...
    "sc2": "If we choose the rear element of the queue to be in position 0, then dequeue operations require only <em>O(1)</em> time because the front element of the queue (the one being removed) is the last element in the array.",
    "sc3": "However, enqueue operations will require <em>O(n)</em> time, because the n elements currently in the queue must each be shifted one position in the array.",
    "sc4": "If instead we chose the rear element of the queue to be in position <code>listsize-1</code>, then an enqueue operation is equivalent to an append operation on a list. This requires only <em>O(1)</em> time.",
    "sc5": "But now, a dequeue operation requires <em>O(n)</em> time, because all of the elements must be shifted down by one position to retain the property that the remaining <em>n–1</em> queue elements reside in the first <em>n–1</em> positions of the array."
  };
