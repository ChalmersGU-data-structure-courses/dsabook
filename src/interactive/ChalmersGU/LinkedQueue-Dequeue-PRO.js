...
(function() {
  ...
  var LinkedQueueDequeuePRO = {
    ...
    pclick: function(pointer) {
      ...
      status = 3;
      LinkedQueueDequeuePRO.userInput = true;
    },
    ...
    connect: function(obj1, obj2) {
      var i;
      if (obj1 === obj2) { return; }
      LinkedQueueDequeuePRO.connection(obj1, obj2);
      obj1.odsa_next = obj2;
      obj1._next = obj2;
    ...
    clickHandler: function(e) {
      ...
      if ((x > 31) && (x < 42) && (y > 0) && (y < 31)) { // In pointer area
        ...
        if (status === 2) {
          toNode = this;
          LinkedQueueDequeuePRO.connect(fromNode, toNode);
          $("#" + fromNode.id() + " .jsavpointerarea:first").removeClass("bgColor");
    ...
    makenull: function() {
      if (status === 2) {
        ...
        LinkedQueueDequeuePRO.addTail(fromNode);
        fromNode = null;
        status = 0;
      }
    },
    ...
    initJSAV: function(size) {
      ...
      $("#makenull").click(function() { LinkedQueueDequeuePRO.makenull(); });
    ...
    checkAnswer: function(arr_size) {
      ...
      var curr = jsavList.get(0);
      if (curr.odsa_next !== null) { return false; }
  ...
  function reset() {
    ...
    leftMargin = 40;
    topMargin = 60;
    LinkedQueueDequeuePRO.userInput = false;
    connections = [];
    selected_node = null;
    copyFrom = null;
    status = 0;
    ...
    if ($("#LinkedQueue-Dequeue-PRO")) { $("#LinkedQueue-Dequeue-PRO").empty(); }

    // Set up the display
    av = new JSAV("LinkedQueue-Dequeue-PRO");
    ...
    jsavList.layout();
    front = LinkedQueueDequeuePRO.setPointer("front", jsavList.get(0));
    rear = LinkedQueueDequeuePRO.setPointer("rear", jsavList.get(listSize - 1));
    ...
    answerOrderArr.splice(0, 1);
    answerOrderArr.length = listSize - 1;
    answCopyFrom = jsavList.get(0);
    answFrontNode = jsavList.get(1);
    answRearNode = jsavList.get(listSize - 1);
    av.displayInit();
    av.recorded();

    // Bind click event handlers.
    front.click(LinkedQueueDequeuePRO.pclick);
    rear.click(LinkedQueueDequeuePRO.pclick);
    returnArr.click(LinkedQueueDequeuePRO.copyHandler);
    jsavList.click(LinkedQueueDequeuePRO.clickHandler);

    LinkedQueueDequeuePRO.userInput = false;
  }

  window.LinkedQueueDequeuePRO = window.LinkedQueueDequeuePRO || LinkedQueueDequeuePRO;
}());
