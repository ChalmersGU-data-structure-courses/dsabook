...
(function() {
  ...
  function reset() {
    ...
    if ($("#StaticArrayListRemovePRO")) { $("#StaticArrayListRemovePRO").empty(); }

    // Set up the display
    // (Re-)initialize the displayed array object
    av = new JSAV("StaticArrayListRemovePRO");
    jsavArr = av.ds.array(cloneArr, {indexed: true, center: false, top: 0, left: 1});
    returnArr = av.ds.array(["null"], {left: 140, top: 110});
    av.label(`remove(${delPosition}) &nbsp;&rarr;`, {left: 20, top: 115});
    av.displayInit();
    av.recorded();

    jsavArr.click(clickHandler);  // (Re-)Bind click handler
    returnArr.click(copyHandler); // (Re-)Bind click handler
    staticArrayListRemovePRO.userInput = false;
    selected_index = -1;
  }
  ...
  function clickHandler(index) {
    ...
    staticArrayListRemovePRO.userInput = true;
  }

  window.staticArrayListRemovePRO = window.staticArrayListRemovePRO || staticArrayListRemovePRO;
}());
