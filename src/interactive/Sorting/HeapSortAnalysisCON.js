...
$(document).ready(function() {
  ...
  var interpret = {
	  "Slide 1": "The first step in heapsort is to heapify the array. This will cost <em>O(n)</em> running time for an array of size <em>n</em>.<br> Consider the following structure of a Max Heap",
	  ...
	  "Slide 8": "This will requires <em>log(i)</em> amount of work, where <em>i</em> is the index of the current last position in the heap",
	  ...
	  "Slide 10": "Since most of the heap nodes are located towards the bottom of the heap, the distance from the root to the current last position in the heap will be in most cases <em>log(i)</em>, thus this summation is bounded by <em>n log(n)</em>",
	  "Slide 11": "Accordingly, the total running time of heapsort is <em>O(n + n log(n)) = O(n log(n))</em>",
	  "Slide 12": "If all key values were equal, then Heapsort would cost <em>O(n)</em> in the best case because every call to removemax would result in calls to siftdown that complete in constant time because the new root value never swaps with its children."
  };
  ...
  bh = av.ds.binheap(arr, {left: 100, top: 20, nodegap: 15});
  bh.element.hide();
  bh.layout();
  av.displayInit();
  ...
  var label = av.label("<em>&Sigma;<sub>i&le;n</sub> log(i)<em>",  {top: "-10px", left: "10px"}).css({"font-size": "16px", "text-align": "center"});
