...
Python lists are so-called [dynamic arrays]{.term} -- we will discuss how they work in @sec:sequences:dynamic-arrays.
:::
...
This is, for example, done already in @sec:sorting-1:binary-search where we introduce the *binary search* algorithm.
...
Here is an example of how to define a dynamic array (see @sec:sequences:dynamic-arrays):
...
Another example datatype are *linked list nodes* (see @sec:sequences:linked-stacks):
...
Here's a final example of *binary tree nodes* (see @sec:trees:binary-trees):
