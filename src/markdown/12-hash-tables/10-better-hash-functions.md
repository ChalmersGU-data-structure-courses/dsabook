...
computation modulus the table size to be safe.
So if we have keys that are bigger than 999,
we can still make sure that the result is in the range 0 to 9
by taking the modulus of 10 at the end.

Binning looks at the opposite part of the key value from the modulus
function. Modulus, for a power of two, looks at the low-order
...
contrast, if we use the modulus function, then we are assigning to any given
