...
- Prio 3: discuss the relation with a tree
    - (that is, why is this section in this chapter?)
...
(that is, some number of pointers). The assignment is random, using a
...
and yielding poor (that is, $O(n)$) performance during search, because
