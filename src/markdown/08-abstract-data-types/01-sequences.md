...
Then we will call the ADT an *interface*, and this interface will *extend* the basic *Collection* interface.
    ...
    interface Collection:
        isEmpty() -> Bool   // Tells if the collection contains no elements.
...
Or we could specify it clearer, and use more self-explaining names:
    ...
    interface Deque of T extends Collection:
        addFirst(x)         // Adds x at the front of the list.
        addLast(x)          // Adds x at the end of the list.
        removeFirst() -> T  // Removes the first element.
        removeLast() -> T   // Removes the last element.
        getFirst() -> T     // Looks at the first element.
        getLast() -> T      // Looks at the last element.
...
General lists can in turn be seen as an extension of deques, where we can insert, remove, update and look at an arbitrary position:
    ...
    interface List of T extends Deque:
        insert(i, x)    // Inserts x at position i, increasing the size of the list.
        remove(i)       // Deletes the element at position i, decreasing the size of the list.
